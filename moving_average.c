...
main (int argc, char *argv[]) {
	...
	int i, j, k, t, n_atoms, n_bins, bin_dist, bin_angle_phi, bin_angle_theta, n, ***hist_dist_angle, counter_line[9], item, nk[3], n_of_param;
	double putnumber, popc_data[400][50][3], sdpc_data[207][134][3], water_data[12820][3][3], sod_data[2][1][3], eau_data, chol_data[40][29][3];
	...
	double *final_count_water, *el_density_chol, **count_water, **count_popc, *final_count_popc, *final_count_chol, **count_chol;
	...
	double vect1_x, vect1_y, vect1_z, arccos, distance1, angle_final, dist_res, counter_ala, counter_leu, *dist_res_ala, *dist_res_ala2, *dist_res_ala3, *dist_res_leu; 
	double *dist_res_ala4, *dist_res_ala5, *dist_res_ala6, *dist_res_ala7, *dist_res_ala8, *dist_res_ala9, ala_in_window, leu_in_window;
	...
	int *water_mol_number, water_atom_number;
        ...
        int *sod_mol_number, sod_atom_number, frame;

	char first[80];

	FILE *inptr, *incholptr, *eldppcptr, *elwatptr, *paraptr, *elcholptr, *outptr, *leuptr;
	...
	for (i=0; i<(n_of_files-window); ++i) {
		...
		for (k=0; k<n_of_param; ++k) {
			for (j=i; j<(i+window); ++j) {
				data_in_window[k] = data_in_window[k] + data_in_file[j][k];	
			}	
		}
		if (data_in_window[2] < min_z) min_z = data_in_window[2];
		if (data_in_window[3] < min_z) min_z = data_in_window[3];
		if (data_in_window[4] < min_z) min_z = data_in_window[4];
		if (data_in_window[13] < min_z) min_z = data_in_window[13];
//		if (i % 5 ==0) {
		for (j=0; j<n_of_param; ++j) {
			fprintf (outptr, "%lf ", data_in_window[j]/window);
		}
		fprintf (outptr, "%lf\n", min_z/window);
//		}
	}
