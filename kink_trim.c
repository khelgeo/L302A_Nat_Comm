...
main (int argc, char *argv[]) {
	...
	double popc_vel, chol_vel, water_vel, sod_vel, protein_vel, eau_vel, new_popc_data[207][52][3], new_chol_data[50][49][3], min_z, max_z, junk, jk1, jk2, jk3, jk4;
	double *bin_size_dist, *final_count_water, *el_density_chol, **count_water, **count_popc, *final_count_popc, *final_count_chol, **count_chol, *kink_data3, *kink_data4;
	...
	double box_x, box_y, box_z, dist_x, dist_y, dist_z, *r, distance, const_fact, rlower, rupper, nideal, ***grdf, rho, *average_val, phi, *dz, delta_z, *stand_dev, grand_average;
	...
	double *min, *max, *dist_res_ala4, *dist_res_ala5, *dist_res_ala6, *dist_res_ala7, *dist_res_ala8, *dist_res_ala9, ala_in_window, leu_in_window, *count_angle1, *count_angle2;
	...
	FILE *paramptr[24], *inptr, *incholptr, *eldppcptr, *elwatptr, *paraptr, *elcholptr, *outptr, *leuptr, *inAVEptr;
	...
	kink_data3 = (double *) malloc(n_of_files*(sizeof(double)));
	kink_data4 = (double *) malloc(n_of_files*(sizeof(double)));
	...
	count_angle1 = (double *) malloc(n_of_param*(sizeof(double)));
	count_angle2 = (double *) malloc(n_of_param*(sizeof(double)));
	...
	for (i=0; i<n_of_files; ++i) {
		fscanf (inptr, "%lf %lf %lf %lf %lf %lf\n", &kink_data3[i], &jk1, &jk2, &kink_data4[i], &jk3, &jk4);
		if (kink_data3[i]<60 && kink_data4[i]<60) printf ("%lf %lf %lf %lf %lf %lf\n", kink_data3[i], jk1, jk2, kink_data4[i], jk3, jk4);
	}
