...
main (int argc, char *argv[]) {
	...
	double popc_vel, chol_vel, water_vel, sod_vel, protein_vel, eau_vel, new_popc_data[207][52][3], new_chol_data[50][49][3], min_z, max_z, junk, *max_value, *max_bin_num;
	...
	for (i=0; i<n_of_files; ++i) {
		for (j=0; j<n_of_param; ++j) {
			fscanf (inptr, "%lf %lf ", &junk, &data_in_file[i][j]);
		}
		fscanf (inptr, "\n");
	}
	...
	for (j=0; j<n_of_param; ++j) {
        	for (i=0; i<n_of_files; ++i) {
                	if (data_in_file[i][j] <= 7.0) average_val[j] = average_val[j] + 1.0;
        	}
	}
	...
	for (k=0; k<n_of_param; ++k) {
                	fprintf (paramptr[k], "%lf\n", average_val[k]/(n_of_files*1.0));
	}
