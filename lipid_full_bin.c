...
main (int argc, char *argv[]) {
	...
	double **data_in_file_pope_tail, **data_in_file_popg_tail;
	double **data_in_file_pope_head, **data_in_file_popg_head;
	...
	sod_mol_number = (int *) malloc(n_sod*(sizeof(int)));
        ...
        data_in_file_pope_tail = (double **) malloc(n_of_files * sizeof(double *));
        data_in_file_pope_tail[0] = malloc(n_of_files * n_of_param * sizeof(double));
        for(i = 1; i < n_of_files; ++i)
                data_in_file_pope_tail[i] = data_in_file_pope_tail[0] + i * n_of_param;

        data_in_file_popg_tail = (double **) malloc(n_of_files * sizeof(double *));
        data_in_file_popg_tail[0] = malloc(n_of_files * n_of_param * sizeof(double));
        for(i = 1; i < n_of_files; ++i)
                data_in_file_popg_tail[i] = data_in_file_popg_tail[0] + i * n_of_param;

        data_in_file_pope_head = (double **) malloc(n_of_files * sizeof(double *));
        data_in_file_pope_head[0] = malloc(n_of_files * n_of_param * sizeof(double));
        for(i = 1; i < n_of_files; ++i)
                data_in_file_pope_head[i] = data_in_file_pope_head[0] + i * n_of_param;

        data_in_file_popg_head = (double **) malloc(n_of_files * sizeof(double *));
        data_in_file_popg_head[0] = malloc(n_of_files * n_of_param * sizeof(double));
        for(i = 1; i < n_of_files; ++i)
                data_in_file_popg_head[i] = data_in_file_popg_head[0] + i * n_of_param;
	...
	for (i=0; i<n_of_files; ++i) {
		for (j=0; j<n_of_param; ++j) {
			fscanf (inptr, "%lf %lf %lf %lf", &data_in_file_pope_tail[i][j], &data_in_file_popg_tail[i][j], &data_in_file_pope_head[i][j], &data_in_file_popg_head[i][j]);
			data_in_file[i][j] = data_in_file_pope_tail[i][j] + data_in_file_popg_tail[i][j] + data_in_file_pope_head[i][j] + data_in_file_popg_head[i][j];
		}
		fscanf (inptr, "\n");
	}
	...
	for (j=0; j<n_of_param; ++j) {
        	round_min[j] = floor(min[j]) - 5;
        	round_max[j] = ceil(max[j]) + 5;
