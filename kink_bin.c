...
main (int argc, char *argv[]) {
	...
	for (i=0; i<n_of_files; ++i) {
        ...
        for (j=0; j<n_of_param; ++j) {
                for (i=0; i<n_of_files; ++i) {
			if (data_in_file[i][j] < 90) {
                        average_val[j] = average_val[j] + data_in_file[i][j];
				count_angle1[j] = count_angle1[j] + 1;
			}
                }
                average_val[j] = average_val[j]*1.0/(count_angle1[j]*1.0);
        }
        ...
        for (j=0; j<n_of_param; ++j) {
			...
			if (data_in_file[i][j] < 90) {
                ...
                stand_dev[j] = sqrt(stand_dev[j]/(count_angle1[j]*1.0));
        }


//        for (j=0; j<n_of_param; ++j) {
  //              for (i=0; i<n_of_files; ++i) {
  //                      if (data_in_file[i][j] > grand_average) count_angle1[j] = count_angle1[j] + 1;
  //                      if (data_in_file[i][j] <= grand_average) count_angle2[j] = count_angle2[j] + 1;
  //              }
  //              count_angle1[j] = count_angle1[j]/(n_of_files*1.0);
  //              count_angle2[j] = count_angle2[j]/(n_of_files*1.0);
  //      }
...
//                	fprintf (paramptr[k], "#  %lf  %lf %lf\n", average_val[k], stand_dev[k], average_val[k]-grand_average);
